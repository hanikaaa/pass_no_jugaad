...
import { type NavProps, type Page, type UserRole } from '../data/events';

interface BottomNavProps extends NavProps {
  activeRole?: UserRole;
}

export default function BottomNav({ navigate, currentPage, activeRole = 'buyer' }: BottomNavProps) {
  const isSuperAdmin = activeRole === 'super_admin';
  const isOrganiser = activeRole === 'organiser';

  const userTab: { label: string; page: Page } = isSuperAdmin
    ? { label: 'Admin', page: 'admin-dashboard' }
    : isOrganiser
    ? { label: 'Hub', page: 'organiser-dashboard' }
    : { label: 'Mine', page: 'my-requests' };
  ...
  const tabs: { label: string; page: Page; icon: (active: boolean) => React.ReactElement }[] = [
    ...
    },
    {
      ...
      icon: (a) => (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={a ? '#C1440E' : '#9A8B82'} strokeWidth={1.8}>
          {isSuperAdmin ? (
            <path d="M12 2l3 6 6 1-4.5 4.5 1 6.5-5.5-3-5.5 3 1-6.5L3 9l6-1z" strokeLinecap="round" strokeLinejoin="round" fill={a ? '#C1440E' : 'none'} />
          ) : (
            <>
  ...
  const active = (p: Page) =>
    ...
    (userTab.page === 'admin-dashboard' && currentPage === 'admin-dashboard') ||
    (userTab.page === 'my-requests' && currentPage === 'my-requests');
